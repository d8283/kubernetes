...
1. 创建namespace
    ...
    sed -i "s#databases#YOUR_NAMESPACE#g" *
...
4. 创建slot
	...
	# 等待所有pod启动完毕后，直接执行以下命令。
        ...
        for i in `kubectl get po -n databases -o wide | awk  '{print $6}' | grep -v IP`; do v="$v $i:6379";done

        kubectl exec -ti redis-cluster-ss-5 -n databases -- redis-trib.rb create --replicas 1 $v
    ````

5. 查看状态

    ````
        [root@k8s-master01 ~]# kubectl exec -ti redis-cluster-ss-0 -n databases -- redis-cli cluster nodesf9527e2ced3c472caabe3f815d87531e82e75049 172.168.5.174:6379@16379 master - 0 1541693210490 2 connected 5461-10922
        ...
        [root@k8s-master01 ~]# kubectl exec -ti redis-cluster-ss-0 -n databases -- redis-cli cluster info
        cluster_state:ok
